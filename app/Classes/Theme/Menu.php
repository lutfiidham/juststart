...
class Menu
{
    ...
    public static function renderVerMenu($item, $parent = null, $rec = 0, $singleItem = false)
    {
        ...
        if (isset($item['separator'])) {
            ...
            if (isset($item['submenu']) && count($item['submenu']) > 0 && self::isActiveVerMenuItem($item, request()->path())) {
            ...
            if (isset($item['parent'])) {
                ...
                echo '<a ' . $target . ' href="' . $url . '" class="menu-link ' . (isset($item['submenu']) && count($item['submenu']) > 0 ? 'menu-toggle' : '') . '">';
            }
    ...
    {
        if (isset($item['redirect']) && Str::contains($page, $item['redirect'])) {
            return false;
        }

        self::checkRecursion($rec);

        if (isset($item['page']) && Str::contains($page, $item['page'])) {
            return true;
        }
    ...
    public static function renderMenuPermissionTable($menus, $parentMenu = null, $level = 0)
    {
        $html = '';
        foreach ($menus as $index => $menu) {
            ...
            <tr>
                <td>
                    <div class="checkbox-inline ml-' . ($level * 10) . '">
                        <label class="checkbox">
                            <input type="checkbox" class="menu-checkbox" id="menu-' . $menu->id . '" name="menu-' . $menu->id . '" value="' . $menu->id . '" data-id="' . $menu->id . '" data-parent_id="' . ($parentMenu ? $parentMenu->id : '') . '">
                            <span></span>' . $menu->title . '
                        </label>
                    </div>
                </td>
                <td>' . self::renderPermissionCheckbox($menu->permissions, $menu->id) . '</td>
            </tr>';

            if ($menu->children) {
                $html .= self::renderMenuPermissionTable($menu->children, $menu, ($level + 1));
            }
        }
    ...
    private static function renderPermissionCheckbox($permissions, $menuId)
    {
        $html = '';
        ...
        foreach ($permissions as $index => $permission) {
            $html .= '
            <label class="checkbox">
                <input class="permission-checkbox" type="checkbox" data-menu_id="' . $menuId . '" data-id="' . $permission->id . '" value="' . $permission->id . '" name="permission-' . $permission->id . '"  id="permission-' . $permission->id . '">
